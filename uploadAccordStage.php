...
$date = date("d-m-Y");
        ...
        for ($i = 0; $i < count($_FILES['accordStage']['name']); $i++) {//Boucle pour avoir chaque élement de l'array

            $directory = "img/accord/".$idEtudiant."/";
            $file = glob($directory."*");

            $countfile = count($file);
            
            $target_path = "img/accord/".$idEtudiant."/accord".$countfile; // Declaration du chemain de l'upload

            $validextensions = array("jpeg", "jpg", "png","pdf");  //Extensions accptées
            $ext = explode('.', basename($_FILES['accordStage']['name'][$i]));//explode le fichier a partir du (.)
        ...
        if (($_FILES["accordStage"]["size"][$i] < 1000000) //Approx. 1000kb peuvent être upload
                     && in_array($file_extension, $validextensions)) {
            if (move_uploaded_file($_FILES['accordStage']['tmp_name'][$i], $target_path)) {//verifie que les fichier on bien été transferer
